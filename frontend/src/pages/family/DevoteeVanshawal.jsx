...
import { 
  FiSearch, FiPlus, FiUser, FiMapPin, FiActivity, FiCheck, FiChevronRight, 
  FiChevronLeft, FiX, FiLayers, FiFilter, FiTrendingUp, FiInfo, FiCalendar, FiClock
} from 'react-icons/fi';
...
const DevoteeVanshawal = () => {
  ...
  useEffect(() => {
    ...
    const delayDebounce = setTimeout(async () => {
      try {
        ...
        const res = await api.get(`/family/search-suggestions?q=${encodeURIComponent(searchQuery)}`, {
  ...
  const fetchCities = async (stateVal) => {
    try {
      const res = await api.get(`/family/filters/cities?state=${encodeURIComponent(stateVal)}`);
  ...
  const fetchVillages = async (stateVal, cityVal) => {
    try {
      const res = await api.get(`/family/filters/villages?state=${encodeURIComponent(stateVal)}&city=${encodeURIComponent(cityVal)}`);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 print:hidden">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight flex items-center gap-2">
            <FaUserFriends className="text-saffron-500" /> Devotee Vanshawal Portal
          </h1>
          <p className="text-slate-500 font-medium text-sm mt-1">
            Browse state-wise, city-wise lineages, configure relationships, and discover devotee families.
          </p>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-gray-150 rounded-[2rem] p-6 sm:p-8 shadow-sm space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-wider mb-2">State Filter</label>
                <select 
                  value={selectedState} 
                  onChange={(e) => {
                    ...
                    setSelectedCity("");
                    setSelectedVillage("");
                    updateUrlParams({ state: e.target.value, city: "", village: "", page: 1 });
                  }} 
              ...
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-wider mb-2">City Filter</label>
                <select 
                  ...
                  onChange={(e) => {
                    ...
                    setSelectedVillage("");
                    updateUrlParams({ city: e.target.value, village: "", page: 1 });
                  }} 
              ...
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-wider mb-2">Village Filter</label>
                <select 
            ...
            <div className="relative" ref={suggestionsRef}>
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-wider mb-2">Global Search</label>
                ...
                <div className="flex gap-2">
                  <button 
                    ...
                    className={`px-5 py-3 rounded-xl text-xs font-black uppercase tracking-wider border flex items-center justify-center gap-2 transition-colors ${
                      isAdvancedOpen ? 'bg-saffron-50 border-saffron-300 text-saffron-700' : 'bg-slate-50 border-slate-200 text-slate-600 hover:bg-slate-100'
                    }`}
                  >
                    <FiFilter /> Advanced
                  </button>
              ...
              <AnimatePresence>
                {showSuggestions && (suggestions.length > 0 || suggestionsLoading) && (
                  ...
                  >
                    {suggestionsLoading && (
                      <div className="px-4 py-3 text-xs text-slate-400 font-medium text-center">Loading suggestions...</div>
                    )}
                    {suggestions.map((item, idx) => (
                      <div
                        ...
                        className={`p-4 text-left cursor-pointer transition-colors flex justify-between items-center ${
                          idx === activeSuggestionIndex ? 'bg-saffron-50/50' : 'hover:bg-slate-50'
                        }`}
                      >
                        <div>
                          <p className="font-bold text-slate-800 text-xs flex items-center gap-1">
                            {item.isFamilyHead ? <FaCrown className="text-saffron-500" /> : <FiUser className="text-slate-400" />}
                            {item.name}
                          </p>
              ...
              {isAdvancedOpen && (
                ...
                >
                  <div className="pt-6 border-t border-slate-100 grid grid-cols-1 sm:grid-cols-4 gap-4">
                    <div>
                      <label className="block text-[10px] font-black text-slate-400 uppercase tracking-wider mb-1.5">Surname</label>
                      <input 
                        type="text" 
                        value={surnameFilter} 
                    ...
                    <div>
                      <label className="block text-[10px] font-black text-slate-400 uppercase tracking-wider mb-1.5">Min Family Members</label>
                      <input 
                        type="number" 
                        value={minMembers} 
                    ...
                    <div>
                      <label className="block text-[10px] font-black text-slate-400 uppercase tracking-wider mb-1.5">Max Family Members</label>
                      <input 
                        type="number" 
                        value={maxMembers} 
                    ...
                    <div>
                      <label className="block text-[10px] font-black text-slate-400 uppercase tracking-wider mb-1.5">Generations Count</label>
                      <input 
                        type="number" 
          ...
          {selectedCity && (
            <div className="bg-slate-50 border border-slate-200/60 p-6 rounded-3xl flex flex-col sm:flex-row justify-between items-center gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="font-black text-slate-800 text-xs uppercase tracking-wider">{selectedCity} Family Summary</h4>
          ...
          <div className="space-y-6">
            ...
            <div className="flex justify-between items-center">
              <h2 className="text-lg font-black text-slate-800 uppercase tracking-wide">Search Results</h2>
              ...
              <div className="flex gap-3">
                ...
                >
                  ...
                  <option value="generationCount-desc">Generation Count: High First</option>
                </select>
              </div>
            </div>
            ...
            {searching ? (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-8 space-y-4">
                  <h3 className="text-xs font-black text-slate-400 uppercase tracking-wider">Families ({filteredCityResults.length})</h3>
                    ...
                    >
                      ...
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div className="flex gap-4">
                          ...
                          <div>
                            <h4 className="font-black text-slate-800 text-sm">{head.name}'s Family</h4>
                            <p className="text-[10px] text-slate-400 font-bold mt-0.5">Family ID: {head.familyId || head._id.substring(0,8).toUpperCase()}</p>
                            
                            <div className="flex flex-wrap gap-x-4 gap-y-1.5 mt-2.5">
                              <span className="text-[10px] text-slate-500 font-medium flex items-center gap-1"><FiMapPin className="text-slate-400" /> {head.city || 'N/A'}, {head.state || 'N/A'}</span>
                              <span className="text-[10px] text-slate-500 font-medium flex items-center gap-1"><FiLayers className="text-slate-400" /> Gen: {head.generationCount || 1}</span>
                              <span className="text-[10px] text-slate-500 font-medium flex items-center gap-1"><FiActivity className="text-slate-400" /> Members: {head.memberCount || 1}</span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex gap-2 w-full sm:w-auto">
                          <button 
                            onClick={() => updateUrlParams({ viewFamily: head.familyId || head._id })}
                          ...
                          <button 
                            onClick={() => { setSelectedMemberId(head._id); }}
                      ...
                      <div className="border-t border-slate-100 mt-4 pt-3 flex justify-between text-[8px] font-black text-slate-400 uppercase tracking-widest">
                        <span className="flex items-center gap-1"><FiCalendar /> Created: {new Date(head.createdAt).toLocaleDateString()}</span>
                        <span className="flex items-center gap-1"><FiClock /> Updated: {new Date(head.updatedAt).toLocaleDateString()}</span>
                      </div>
                    </div>
                  ))}
                ...
                <div className="lg:col-span-4 space-y-4">
                  <h3 className="text-xs font-black text-slate-400 uppercase tracking-wider">Devotees ({devoteeResults.length})</h3>
                  ...
                  <div className="bg-white border border-gray-150 rounded-[2rem] p-4 divide-y divide-slate-100">
                    {devoteeResults.map(dev => (
                      ...
                      >
                        <div>
                          <p className="font-bold text-slate-800 text-xs">{dev.name}</p>
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            ...
            >
              ...
              <form onSubmit={handleCreateFamily} className="p-6 space-y-4 overflow-y-auto flex-1 custom-scrollbar">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-wider mb-1.5">City</label>
                    <input type="text" value={formData.city} onChange={e=>setFormData({...formData, city: e.target.value})} className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:bg-white focus:outline-none focus:ring-1 focus:ring-saffron-500" placeholder="City" />
                  </div>
                </div>
