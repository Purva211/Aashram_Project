...
const RegisterDevotee = () => {
  ...
  useEffect(() => {
    if (formData.state && INDIA_STATES_CITIES[formData.state]) {
      setAvailableCities(INDIA_STATES_CITIES[formData.state].sort());
      // Reset city if the new state doesn't have the selected city
      if (!INDIA_STATES_CITIES[formData.state].includes(formData.city)) {
  ...
  const checkDuplicate = async (field, value) => {
    ...
    const setStatus = field === "email" ? setEmailStatus : setMobileStatus;
    setStatus("checking");
    try {
  ...
  const handleStep1Submit = async (e) => {
    ...
    if (emailStatus === "taken") return setError("Email is already taken.");
    if (mobileStatus === "taken") return setError("Mobile number is already taken.");
  ...
  const handleOtpSubmit = async (e) => {
    ...
    try {
      await api.post("/auth/register/verify-email", { email: formData.email, otp });
  ...
  const handleRegisterSubmit = async (e) => {
    e.preventDefault();
    if (formData.role === 'Devotee' && formData.registerOption === "joinFamily" && !formData.relativeId) {
    ...
    try {
      const { name, email, mobile, password, confirmPassword, role, ...profileData } = formData;
      const payload = {
        email: formData.email,
        profileData,
        registerOption: formData.registerOption,
          ...
          <div className="bg-white/90 backdrop-blur-2xl border border-white shadow-[0_20px_50px_-15px_rgba(0,0,0,0.05),0_0_30px_rgba(255,165,0,0.05)] p-6 sm:p-8 md:p-12 rounded-[2.5rem] relative overflow-hidden group">
            ...
            <div className="flex flex-col items-center mb-8 relative z-10">
              <div className="w-16 h-16 bg-gradient-to-br from-orange-100 to-red-100 border border-orange-200 rounded-full flex items-center justify-center text-orange-600 shadow-md mb-4">
                {step === 1 ? <UserCircle size={32} /> : step === 2 ? <Mail size={32} /> : <FaVenusMars size={28} />}
              </div>
              ...
              <p className="text-slate-500 font-bold text-xs mt-2 text-center uppercase tracking-wider">
                {step === 1 ? "Step 1 of 3: Setup Credentials" : step === 2 ? `Step 2 of 3: OTP sent to ${formData.email}` : "Step 3 of 3: Profile Information"}
              </p>
            </div>
              ...
              <form onSubmit={handleStep1Submit} className="space-y-4 relative z-10">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2">
                    <label className="block text-slate-700 text-[10px] font-black mb-1.5 uppercase tracking-wider ml-1">Role</label>
                    <div className="relative">
                      <Users className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                      <select
                        value={formData.role}
                        onChange={(e) => setFormData({ ...formData, role: e.target.value })}
                        className="w-full bg-slate-50/50 border border-slate-200 text-slate-800 rounded-2xl pl-11 pr-4 py-4 outline-none focus:border-orange-400 focus:bg-white text-xs font-bold transition-all shadow-sm"
                      >
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      required type="text" pattern="[A-Za-z\s]+" title="Name must contain only alphabets and spaces"
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={`w-full bg-slate-50/50 border ${emailStatus === 'taken' ? 'border-red-400' : 'border-slate-200'} text-slate-800 rounded-2xl pl-11 pr-4 py-3.5 outline-none focus:border-orange-400 focus:bg-white text-xs font-bold transition-all shadow-sm`}
                      placeholder="Enter email address"
                    />
                    {emailStatus === 'taken' && <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-bold text-red-500">Taken</span>}
                    {emailStatus === 'available' && <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-bold text-green-500"><Check size={14} /></span>}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      required type="tel" pattern="\d{10}" title="Mobile number must be exactly 10 digits" maxLength={10}
                      value={formData.mobile}
                      onChange={(e) => {
                        setFormData({ ...formData, mobile: e.target.value.replace(/\D/g, "") });
                        if (mobileStatus !== "") setMobileStatus("");
                      }}
                      onBlur={(e) => checkDuplicate("mobile", e.target.value)}
                      className={`w-full bg-slate-50/50 backdrop-blur-sm border ${mobileStatus === 'taken' ? 'border-red-400' : 'border-slate-200'} text-slate-800 rounded-2xl pl-11 pr-4 py-4 outline-none focus:border-orange-400 focus:ring-4 focus:ring-orange-400/10 focus:bg-white transition-all placeholder:text-slate-400 font-bold shadow-sm`}
                      placeholder="Enter mobile number"
                    />
                    {mobileStatus === 'taken' && <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-bold text-red-500">Taken</span>}
                    {mobileStatus === 'available' && <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-bold text-green-500"><Check size={14} /></span>}
                  </div>
                </div>
            ...
            {step === 2 && (
              <form onSubmit={handleOtpSubmit} className="space-y-6 relative z-10 animate-[fadeIn_0.4s_ease-out]">
                ...
                <button
                  disabled={loading || otp.length !== 6} type="submit"
            ...
            {step === 3 && (
              <form onSubmit={handleRegisterSubmit} className="space-y-4 relative z-10 max-h-[65vh] overflow-y-auto pr-2 custom-scrollbar">
                    ...
                    {formData.registerOption === "joinFamily" ? (
                      <div className="space-y-4 bg-orange-50/20 p-4 border border-orange-100 rounded-2xl">
                        ...
                        <div className="flex gap-2">
                          <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search by relative head name, mobile, ID..." className="flex-1 bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold focus:outline-none" />
                          <button type="button" onClick={handleSearchRelatives} className="px-4 py-2 bg-orange-500 text-white rounded-xl text-xs font-black uppercase">Search</button>
                        </div>

                        <div className="grid grid-cols-2 gap-2">
                          <input type="text" value={searchState} onChange={(e) => setSearchState(e.target.value)} placeholder="Filter by State" className="bg-white border border-slate-200 rounded-xl px-3 py-2 text-[11px] font-bold focus:outline-none" />
                          <input type="text" value={searchVillage} onChange={(e) => setSearchVillage(e.target.value)} placeholder="Filter by Village" className="bg-white border border-slate-200 rounded-xl px-3 py-2 text-[11px] font-bold focus:outline-none" />
                        </div>
                        ...
                        {searchResults.length > 0 && (
                          <div className="bg-white border border-slate-200 rounded-xl divide-y divide-slate-100 max-h-36 overflow-y-auto">
                            {searchResults.map((rel) => (
                              <div key={rel._id} onClick={() => selectRelative(rel)} className="p-2.5 flex justify-between items-center cursor-pointer hover:bg-orange-50">
                                <div>
                                  <p className="text-xs font-black text-slate-700">{rel.name} ({rel.devoteeId})</p>
                                  <p className="text-[9px] text-slate-400 font-semibold mt-0.5">Branch: {rel.branch?.name || "N/A"}</p>
                                </div>
                        ...
                        {selectedRelative && (
                          <div>
                            ...
                            <select value={formData.relationshipType} onChange={(e) => handleRelationshipChange(e.target.value)} className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold text-slate-700 focus:outline-none">
                      ...
                      <div className="p-3 bg-orange-500/10 border border-orange-200 text-orange-800 rounded-xl text-xs font-bold">
                        You will be registered as a standalone family head.
                      </div>
                    )}
                  </>
                )}
                  ...
                  {formData.role === 'Devotee' ? (
                    <>
                      <div>
                        ...
                        <input
                          type="text" value={formData.kuldevta} onChange={(e) => setFormData({ ...formData, kuldevta: e.target.value })}
                          className="w-full bg-slate-50/50 border border-slate-200 text-slate-800 rounded-2xl px-4 py-3 text-xs font-bold outline-none" placeholder="Family Deity"
                        />
                      </div>
                      <div>
                        <label className="block text-slate-700 text-[10px] font-black mb-1.5 uppercase tracking-wider ml-1">Aadhaar (Optional)</label>
                        <input
                          type="text" value={formData.aadhaar} onChange={(e) => setFormData({ ...formData, aadhaar: e.target.value })}
                          className="w-full bg-slate-50/50 border border-slate-200 text-slate-800 rounded-2xl px-4 py-3 text-xs font-bold outline-none" placeholder="XXXX-XXXX-XXXX"
                        />
                      </div>
                    </>
                  ) : (
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50/50 border border-slate-200 text-slate-800 rounded-2xl px-4 py-3 text-xs font-bold outline-none" placeholder="Permanent address details"
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-slate-700 text-[10px] font-black mb-1.5 uppercase tracking-wider ml-1">City</label>
                    <select
                      required
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-slate-700 text-[10px] font-black mb-1.5 uppercase tracking-wider ml-1">District (Optional)</label>
                    <input
                      type="text" value={formData.district} onChange={(e) => setFormData({ ...formData, district: e.target.value })}
                      className="w-full bg-slate-50/50 border border-slate-200 text-slate-800 rounded-2xl px-4 py-3 text-xs font-bold outline-none" placeholder="District"
                    />
                  </div>
                  <div>
                    <label className="block text-slate-700 text-[10px] font-black mb-1.5 uppercase tracking-wider ml-1">Taluka (Optional)</label>
                    <input
                      type="text" value={formData.taluka} onChange={(e) => setFormData({ ...formData, taluka: e.target.value })}
                      className="w-full bg-slate-50/50 border border-slate-200 text-slate-800 rounded-2xl px-4 py-3 text-xs font-bold outline-none" placeholder="Taluka"
                    />
                  </div>
                </div>

                <div>
                  <label className="block text-slate-700 text-[10px] font-black mb-1.5 uppercase tracking-wider ml-1">Village (Optional)</label>
                  <input
                    type="text" value={formData.village} onChange={(e) => setFormData({ ...formData, village: e.target.value })}
                    className="w-full bg-slate-50/50 border border-slate-200 text-slate-800 rounded-2xl px-4 py-3 text-xs font-bold outline-none" placeholder="Village"
                  />
                </div>
                ...
                >
                  {loading ? <Loader2 className="animate-spin animate-pulse" /> : "Complete Registration"}
                </button>
              </form>
            )}
          </div>
