...
const Announcements = () => {
  ...
  const {
    ...
    paginatedData,
    totalItems,
  } = useTableFeatures(filteredAnnouncements, [
    "title",
    "subject",
    "message",
    "status",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (audienceSettings.allTrustees)
        payload.audienceType.push("All Trust Members");
      if (audienceSettings.allBranchManagers)
        payload.targetRoles.push("BranchManager");
      if (audienceSettings.allAccountants)
        payload.targetRoles.push("Accountant");
      if (audienceSettings.allDocHandlers)
        payload.targetRoles.push("DocumentAdmin");
  ...
  const handleEdit = (ann) => {
    ...
    setFormData({
      ...
      expiryDate: ann.expiryDate
        ? new Date(ann.expiryDate).toISOString().split("T")[0]
        : "",
    ...
    setAudienceSettings({
      ...
      specificAdmins: admins
        .filter((a) => tUsers.includes(a._id))
        .map((a) => a._id),
      specificTrustees: trustees
        .filter((t) => tUsers.includes(t._id))
        .map((t) => t._id),
      specificBranchManagers: branchManagers
        .filter((b) => tUsers.includes(b._id))
        .map((b) => b._id),
      specificAccountants: accountants
        .filter((a) => tUsers.includes(a._id))
        .map((a) => a._id),
      specificDocHandlers: documentHandlers
        .filter((d) => tUsers.includes(d._id))
        .map((d) => d._id),
    });
    ...
    setAudienceSettings((prev) => {
      if (!isSpecific) {
        ...
        return {
          ...
          ...(isChecked && field === "allTrustees"
            ? { specificTrustees: [] }
            : {}),
          ...(isChecked && field === "allBranchManagers"
            ? { specificBranchManagers: [] }
            : {}),
          ...(isChecked && field === "allAccountants"
            ? { specificAccountants: [] }
            : {}),
          ...(isChecked && field === "allDocHandlers"
            ? { specificDocHandlers: [] }
            : {}),
        };
      } else {
  ...
  if (loading)
    return (
      <div className="h-64 flex items-center justify-center">
        <div className="w-8 h-8 border-4 border-saffron-500 rounded-full border-t-transparent animate-spin"></div>
      </div>
    );

  const totalReads = announcements.reduce(
    (acc, ann) => acc + (ann.totalRead || 0),
    0,
  );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
        <div>
          ...
          <p className="text-slate-500 mt-2 font-medium">
            Enterprise announcement engine with omnichannel distribution.
          </p>
        </div>
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="flex items-center gap-2 px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-black rounded-xl shadow-lg transition-all whitespace-nowrap"
          >
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex bg-white rounded-xl shadow-sm border border-slate-100 p-1 gap-1 w-full md:w-auto">
          <button
            ...
            className={`flex-1 md:flex-none md:px-8 py-2.5 rounded-lg text-sm font-bold transition-all ${activeTab === "list" ? "bg-indigo-600 text-white shadow" : "text-slate-500 hover:bg-slate-50 hover:text-slate-900"}`}
          >
            <FiFileText className="inline mr-2" /> All Broadcasts
          </button>
          <button
            onClick={() => setActiveTab("analytics")}
            className={`flex-1 md:flex-none md:px-8 py-2.5 rounded-lg text-sm font-bold transition-all ${activeTab === "analytics" ? "bg-indigo-500 text-white shadow" : "text-slate-500 hover:bg-slate-50 hover:text-slate-900"}`}
          >
        ...
        {activeTab === "list" && (
          <div className="flex bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
            <button
              ...
              className={`px-4 py-2 text-xs font-bold transition-colors ${ownershipFilter === "all" ? "bg-indigo-50 text-indigo-700" : "text-slate-500 hover:bg-slate-50"}`}
            >
              All
            </button>
            <button
              onClick={() => setOwnershipFilter("mine")}
              className={`px-4 py-2 text-xs font-bold border-l border-gray-200 transition-colors ${ownershipFilter === "mine" ? "bg-indigo-50 text-indigo-700" : "text-slate-500 hover:bg-slate-50"}`}
            >
              My Broadcasts
            </button>
            <button
              onClick={() => setOwnershipFilter("others")}
              className={`px-4 py-2 text-xs font-bold border-l border-gray-200 transition-colors ${ownershipFilter === "others" ? "bg-indigo-50 text-indigo-700" : "text-slate-500 hover:bg-slate-50"}`}
            >
              Others
            </button>
          </div>
            ...
            {paginatedData.length === 0 ? (
              ...
              <div className="grid grid-cols-1 gap-4">
                {paginatedData.map((ann, index) => {
                  const isHighlighted = newlyAddedId
                    ? ann._id === newlyAddedId
                    : index === 0 &&
                      new Date() - new Date(ann.createdAt) < 60000;
                        ...
                        <div className="flex gap-3 items-center">
                          <div className="w-12 h-12 bg-slate-100 rounded-full flex items-center justify-center text-slate-500 font-bold overflow-hidden border border-slate-200 shrink-0">
                            {ann.createdBy?.profilePhoto ? (
                              <img
                                src={`${import.meta.env.VITE_API_URL || "http://localhost:5000"}${ann.createdBy.profilePhoto}`}
                          ...
                          <div>
                            ...
                            <div className="text-xs text-slate-500 mt-1 flex flex-wrap items-center gap-1.5">
                              <span>
                                {timeAgo(ann.publishDate || ann.createdAt)}
                              </span>
                              •
                              <span className="flex items-center gap-1">
                                <FiUsers size={10} /> {ann.audienceType}
                              </span>
                              •
                              <span
                                className={`px-1.5 py-0 text-[9px] font-bold uppercase rounded ${ann.priority === "Urgent" ? "bg-rose-100 text-rose-700" : ann.priority === "Important" ? "bg-amber-100 text-amber-700" : "bg-blue-50 text-blue-600"}`}
                              >
                                {ann.priority}
                              </span>
                              {isHighlighted && (
                                <span className="px-1.5 py-0 text-[9px] font-black uppercase tracking-widest rounded bg-saffron-500 text-white shadow-sm animate-pulse ml-1">
                                  NEW
                                </span>
                              )}
                            </div>
                      ...
                      <div className="mt-3 pt-3 pl-1 md:pl-[60px] border-t border-slate-100 flex flex-wrap items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-3">
                          <div className="flex gap-1.5">
                            {ann.dashboardNotification && (
                              <div
                                className="w-6 h-6 rounded-full flex items-center justify-center text-[10px] bg-slate-100 text-slate-500 hover:bg-blue-50 hover:text-blue-500 transition-colors"
                                title="Dashboard"
                              >
                                <FiBell />
                              </div>
                            )}
                            {ann.emailIntegration && (
                              <div
                                className="w-6 h-6 rounded-full flex items-center justify-center text-[10px] bg-slate-100 text-slate-500 hover:bg-indigo-50 hover:text-indigo-500 transition-colors"
                                title="Email"
                              >
                                <FiMail />
                              </div>
                            ...
                            {ann.smsIntegration && (
                              <div
                                className="w-6 h-6 rounded-full flex items-center justify-center text-[10px] bg-slate-100 text-slate-500 hover:bg-orange-50 hover:text-orange-500 transition-colors"
                                title="SMS"
                              >
                                <FiSmartphone />
                              </div>
        ...
        {activeTab === "analytics" && (
          ...
          >
            ...
            <p className="text-slate-500 font-medium max-w-md mx-auto">
              Track Dashboard Read Status, Email settings across your enterprise
              notifications.
            </p>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mt-10">
              ...
              <div className="p-6 bg-blue-50 rounded-xl border border-blue-100">
                ...
                <p className="text-4xl font-black text-blue-700">
                  {announcements.filter((a) => a.emailIntegration).length}
                </p>
              </div>
            </div>
          </motion.div>
              ...
              >
                ...
                <div className="flex items-center justify-between p-6 border-b border-slate-100 bg-slate-50 shrink-0">
                  <div>
                    <h3 className="text-xl font-black text-slate-900">
                      {editingId ? "Edit Broadcast" : "Compose New Broadcast"}
                    </h3>
                    <p className="text-sm text-slate-500 mt-1">
                      {editingId
                        ? "Update the details below."
                        : "Fill out the details below to distribute your message."}
                    </p>
                  </div>
                  <button
                  ...
                  >
                      ...
                      <div className="space-y-5">
                        <div>
                          ...
                          <input
                            ...
                            className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 font-bold focus:bg-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition-all"
                            placeholder="e.g., Upcoming Temple Renovation"
                          />
                        </div>
                        <div>
                          <label className="block text-xs font-bold text-slate-500 mb-2 uppercase tracking-widest">
                          ...
                          <input
                            ...
                            className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-700 focus:bg-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition-all"
                        ...
                        <div>
                          ...
                          <textarea
                            ...
                            className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-700 focus:bg-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition-all resize-none"
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div className="space-y-4 p-4 bg-slate-50 rounded-xl border border-slate-100">
                          <h4 className="font-bold text-slate-900 flex items-center gap-2">
                            <FiClock className="text-indigo-500" /> Publishing
                          </h4>
                          ...
                          <div>
                            ...
                            <select
                              ...
                              className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-700 outline-none focus:border-indigo-500"
                            >
                          ...
                          <div className="space-y-2">
                            <label className="flex items-center justify-between cursor-pointer p-2.5 bg-white border border-slate-200 rounded-lg hover:border-blue-300 transition-colors">
                              <span className="flex items-center gap-2 text-xs font-bold text-slate-700">
                                <FiBell className="text-blue-500" /> Dashboard
                                Push
                              </span>
                              <input
                                type="checkbox"
                                checked={formData.dashboardNotification}
                                onChange={(e) =>
                                  setFormData({
                                    ...formData,
                                    dashboardNotification: e.target.checked,
                                  })
                                }
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="space-y-4 p-4 bg-slate-50 rounded-xl border border-slate-100 h-full">
                        <h4 className="font-bold text-slate-900 flex items-center gap-2">
                          <FiUsers className="text-saffron-500" /> Audience
                          Targeting
                        </h4>
                        <p className="text-xs text-slate-500">
                          Target specific roles. You can select an entire group
                          or specific members within that group.
                        </p>
                          ...
                          <div className="bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden">
                            <div className="p-3 border-b border-slate-100 bg-slate-50/50 flex flex-col md:flex-row md:items-center justify-between gap-2">
                              <label className="flex items-center gap-2 cursor-pointer text-sm">
                                <input
                                  type="checkbox"
                                  checked={audienceSettings.allAdmins}
                                  onChange={() =>
                                    handleAudienceToggle("allAdmins")
                                  }
                              ...
                              <div className="relative">
                                ...
                                <input
                                  type="text"
                                  placeholder="Search specific admin..."
                                  value={searchAdmins}
                                  onChange={(e) =>
                                    setSearchAdmins(e.target.value)
                                  }
                                  className="pl-7 pr-3 py-1.5 text-xs bg-white border border-slate-200 rounded-md outline-none focus:border-indigo-500 w-full md:w-48"
                                />
                              </div>
                            </div>
                            <div className="max-h-40 overflow-y-auto custom-scrollbar p-2 grid grid-cols-1 md:grid-cols-2 gap-1 bg-white">
                              {admins
                                ...
                                .map((a) => (
                                  <label
                                    key={a._id}
                                    className={`flex items-center gap-2 cursor-pointer text-xs hover:bg-slate-50 p-1.5 rounded ${audienceSettings.specificAdmins.includes(a._id) ? "bg-indigo-50/50" : ""}`}
                                  >
                                    <input
                                      type="checkbox"
                                      checked={audienceSettings.specificAdmins.includes(
                                        a._id,
                                      )}
                                      onChange={() =>
                          ...
                          <div className="bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden">
                            <div className="p-3 border-b border-slate-100 bg-slate-50/50 flex flex-col md:flex-row md:items-center justify-between gap-2">
                              <label className="flex items-center gap-2 cursor-pointer text-sm">
                                <input
                                  type="checkbox"
                                  checked={audienceSettings.allTrustees}
                                  onChange={() =>
                                    handleAudienceToggle("allTrustees")
                                  }
                                  className="rounded text-indigo-600 focus:ring-indigo-500 w-4 h-4"
                                />
                                <span className="text-slate-800 font-bold">
                              ...
                              <div className="relative">
                                ...
                                <input
                                  type="text"
                                  placeholder="Search specific trustee..."
                                  value={searchTr}
                              ...
                              {trustees
                                .filter(
                                  (t) =>
                                    ...
                                    (t.name || t.email || "")
                                      .toLowerCase()
                                      .includes(searchTr.toLowerCase()),
                                )
                                .map((t) => (
                                  <label
                                    key={t._id}
                                    className={`flex items-center gap-2 cursor-pointer text-xs hover:bg-slate-50 p-1.5 rounded ${audienceSettings.specificTrustees.includes(t._id) ? "bg-indigo-50/50" : ""}`}
                                  >
                                    <input
                                      type="checkbox"
                                      checked={audienceSettings.specificTrustees.includes(
                                        t._id,
                                      )}
                                      onChange={() =>
                                        handleAudienceToggle(
                          ...
                          <div className="bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden">
                            <div className="p-3 border-b border-slate-100 bg-slate-50/50 flex flex-col md:flex-row md:items-center justify-between gap-2">
                              <label className="flex items-center gap-2 cursor-pointer text-sm">
                                <input
                                  type="checkbox"
                                  checked={audienceSettings.allBranchManagers}
                                  onChange={() =>
                                    handleAudienceToggle("allBranchManagers")
                                  }
                                  className="rounded text-indigo-600 focus:ring-indigo-500 w-4 h-4"
                                />
                                <span className="text-slate-800 font-bold">
                              ...
                              <div className="relative">
                                ...
                                <input
                                  type="text"
                                  placeholder="Search specific manager..."
                                  value={searchBm}
                            ...
                            <div className="max-h-40 overflow-y-auto custom-scrollbar p-2 grid grid-cols-1 md:grid-cols-2 gap-1 bg-white">
                              {branchManagers
                                .filter((b) =>
                                  (b.name || b.email || "")
                                    .toLowerCase()
                                    .includes(searchBm.toLowerCase()),
                                )
                                .map((b) => (
                                  <label
                                    key={b._id}
                                    className={`flex items-center gap-2 cursor-pointer text-xs hover:bg-slate-50 p-1.5 rounded ${audienceSettings.specificBranchManagers.includes(b._id) ? "bg-indigo-50/50" : ""}`}
                                  >
                                    <input
                                      type="checkbox"
                                      checked={audienceSettings.specificBranchManagers.includes(
                                        b._id,
                                      )}
                                      onChange={() =>
                                        handleAudienceToggle(
                          ...
                          <div className="bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden">
                            <div className="p-3 border-b border-slate-100 bg-slate-50/50 flex flex-col md:flex-row md:items-center justify-between gap-2">
                              <label className="flex items-center gap-2 cursor-pointer text-sm">
                                <input
                                  type="checkbox"
                                  checked={audienceSettings.allAccountants}
                                  onChange={() =>
                                    handleAudienceToggle("allAccountants")
                                  }
                                  className="rounded text-indigo-600 focus:ring-indigo-500 w-4 h-4"
                                />
                                <span className="text-slate-800 font-bold">
                              ...
                              <div className="relative">
                                ...
                                <input
                                  type="text"
                                  placeholder="Search specific accountant..."
                                  value={searchAcc}
                            ...
                            <div className="max-h-40 overflow-y-auto custom-scrollbar p-2 grid grid-cols-1 md:grid-cols-2 gap-1 bg-white">
                              {accountants
                                ...
                                .map((a) => (
                                  <label
                                    key={a._id}
                                    className={`flex items-center gap-2 cursor-pointer text-xs hover:bg-slate-50 p-1.5 rounded ${audienceSettings.specificAccountants.includes(a._id) ? "bg-indigo-50/50" : ""}`}
                                  >
                                    <input
                                      type="checkbox"
                                      checked={audienceSettings.specificAccountants.includes(
                                        a._id,
                                      )}
                                      onChange={() =>
                                        handleAudienceToggle(
                          ...
                          <div className="bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden">
                            <div className="p-3 border-b border-slate-100 bg-slate-50/50 flex flex-col md:flex-row md:items-center justify-between gap-2">
                              <label className="flex items-center gap-2 cursor-pointer text-sm">
                                <input
                                  type="checkbox"
                                  checked={audienceSettings.allDocHandlers}
                                  onChange={() =>
                                    handleAudienceToggle("allDocHandlers")
                                  }
                                  className="rounded text-indigo-600 focus:ring-indigo-500 w-4 h-4"
                                />
                                <span className="text-slate-800 font-bold">
                              ...
                              <div className="relative">
                                ...
                                <input
                                  type="text"
                                  placeholder="Search specific handler..."
                                  value={searchDh}
                                  onChange={(e) => setSearchDh(e.target.value)}
                                  className="pl-7 pr-3 py-1.5 text-xs bg-white border border-slate-200 rounded-md outline-none focus:border-indigo-500 w-full md:w-48"
                                />
                              </div>
                            </div>
                            <div className="max-h-40 overflow-y-auto custom-scrollbar p-2 grid grid-cols-1 md:grid-cols-2 gap-1 bg-white">
                              {documentHandlers
                                .filter((d) =>
                                  (d.email || "")
                                    .toLowerCase()
                                    .includes(searchDh.toLowerCase()),
                                )
                                .map((d) => (
                                  <label
                                    key={d._id}
                                    className={`flex items-center gap-2 cursor-pointer text-xs hover:bg-slate-50 p-1.5 rounded ${audienceSettings.specificDocHandlers.includes(d._id) ? "bg-indigo-50/50" : ""}`}
                                  >
                                    <input
                                      type="checkbox"
                                      checked={audienceSettings.specificDocHandlers.includes(
                                        d._id,
                                      )}
                                      onChange={() =>
                                        handleAudienceToggle(
                                    ...
                                    <span className="text-slate-700 truncate">
                                      ...
                                      <span className="text-slate-400">
                                        (
                                        {d.role === "document_admin"
                                          ? "Document Handler"
                                          : d.role || "Document Handler"}
                                        )
                                      </span>
                                    </span>
                                  </label>
                ...
                <div className="p-6 border-t border-slate-100 bg-white shrink-0 flex justify-end gap-4">
                  <button
                    type="button"
                    onClick={() => setIsModalOpen(false)}
                    className="px-6 py-2.5 rounded-xl text-slate-500 hover:bg-slate-50 font-bold transition-colors"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    disabled={
                      submitting || !formData.title || !formData.message
                    }
                    type="submit"
                    className="flex items-center gap-2 px-8 py-2.5 bg-blue-900 hover:bg-blue-800 text-white font-bold rounded-xl shadow-lg shadow-indigo-200 transition-all disabled:opacity-50"
                  >
                    {submitting ? (
                      ...
                      <>
                        <FiSend />{" "}
                        {editingId ? "Update Broadcast" : "Dispatch Broadcast"}
                      </>
                    )}
                  </button>
                </div>
