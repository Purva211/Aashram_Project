  ...
  try {
    ...
    if (user.role === "Admin") {
      ...
      return res
        ...
        .json({
          success: false,
          message: "Unauthorized to manage announcements",
        });
    }
...
exports.createAnnouncement = async (req, res) => {
  try {
    ...
    if (announcement.emailIntegration) {
      try {
        ...
        const addEmails = (docs) => {
          docs.forEach((doc) => {
            if (doc.email) emails.add(doc.email);
          });
        };
        ...
        if (audienceType.includes("All Users")) {
          ...
          addEmails(
            await BranchManager.find({ email: { $exists: true, $ne: "" } }),
          );
          addEmails(
            await Accountant.find({ email: { $exists: true, $ne: "" } }),
          );
          addEmails(
            await DocumentAdmin.find({ email: { $exists: true, $ne: "" } }),
          );
          addEmails(await Admin.find({ email: { $exists: true, $ne: "" } }));
        } else {
          if (audienceType.includes("All Devotees"))
            addEmails(
              await Devotee.find({ email: { $exists: true, $ne: "" } }),
            );
          if (audienceType.includes("All Trust Members"))
            addEmails(
              await Trustee.find({ email: { $exists: true, $ne: "" } }),
            );
          if (audienceType.includes("All Branches"))
            addEmails(
              await BranchManager.find({ email: { $exists: true, $ne: "" } }),
            );
          ...
          if (targetRoles && targetRoles.length > 0) {
            addEmails(
              await Trustee.find({
                systemRole: { $in: targetRoles },
                email: { $exists: true, $ne: "" },
              }),
            );
            if (targetRoles.includes("Admin"))
              addEmails(
                await Admin.find({ email: { $exists: true, $ne: "" } }),
              );
            if (targetRoles.includes("BranchManager"))
              addEmails(
                await BranchManager.find({ email: { $exists: true, $ne: "" } }),
              );
            ...
            if (targetRoles.includes("DocumentAdmin"))
              addEmails(
                await DocumentAdmin.find({ email: { $exists: true, $ne: "" } }),
              );
          }
          if (targetUsers && targetUsers.length > 0) {
            ...
            addEmails(
              await Accountant.find({
                _id: { $in: targetUsers },
                email: { $exists: true, $ne: "" },
              }),
            );
            addEmails(
              await DocumentAdmin.find({
                _id: { $in: targetUsers },
                email: { $exists: true, $ne: "" },
              }),
            );
    ...
    return res
      .status(201)
      .json({ success: true, emailStatus, data: announcement });
  } catch (error) {
  ...
  try {
    ...
    if (
      ...
      return res
        ...
        .json({
          success: false,
          message: "Unauthorized to edit this announcement",
        });
    }
